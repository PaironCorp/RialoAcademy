...
import { ArrowLeft, Globe, Trophy, Cpu, CheckCircle, Award, Wifi, ShieldCheck, Zap, Lock, Unlock } from 'lucide-react';
...
export default function RialoEdgeMission() {
  ...
  const handleComplete = () => {
    if (!isBridgeActive) return; // Защита: нельзя завершить без моста
  ...
  return (
    <main className="min-h-screen p-6 md:p-12 lg:pt-32 flex flex-col items-center relative z-10">
      ...
      <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        <div className="lg:col-span-2 space-y-8">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-[#010101]/80 backdrop-blur-2xl border border-white/10 rounded-[3rem] p-10 md:p-14 shadow-2xl overflow-hidden relative">
                ...
                <p className="text-[#E8E3D5]/70 text-lg md:text-xl font-medium leading-relaxed max-w-xl">Traditional blockchains suffer from the <strong className="text-white">"Oracle Gap"</strong>—a delay when fetching Web2 data.</p>
                ...
                <div className="mt-16 bg-white/[0.02] border border-white/5 rounded-[3rem] p-12 text-center relative overflow-hidden shadow-inner">
                    ...
                    {/* КНОПКА МОСТА С ПОДСКАЗКОЙ */}
                    <button 
                        onClick={toggleBridge} 
                        className={`px-12 py-5 rounded-full font-mono text-[11px] uppercase tracking-[0.4em] transition-all duration-500 border-2 
                        ${isBridgeActive 
                            ? 'bg-[#A9DDD3] text-[#010101] border-[#A9DDD3]' 
                            : 'bg-transparent text-[#A9DDD3] border-[#A9DDD3]/40 hover:bg-[#A9DDD3]/10 animate-pulse shadow-[0_0_20px_rgba(169,221,211,0.2)]'
                        }`}
                    >
                        {isBridgeActive ? "BRIDGE ACTIVE // SYNCING" : "INITIALIZE NATIVE BRIDGE"}
                    </button>
                    {!isBridgeActive && (
                        <p className="mt-4 text-[9px] font-mono text-[#A9DDD3]/40 uppercase tracking-widest animate-bounce">Click to see the magic</p>
                    )}
                </div>
            </motion.div>
        </div>

        {/* SIDEBAR С БЛОКИРОВКОЙ */}
        <div className="space-y-6">
            <div className="bg-[#E8E3D5]/[0.03] border border-white/10 rounded-[2.5rem] p-10 backdrop-blur-md">
                ...
                <div className="space-y-6 mb-10">
                    <div className={`flex items-center justify-between p-5 rounded-2xl border transition-all ${isBridgeActive ? 'bg-[#A9DDD3]/5 border-[#A9DDD3]/20' : 'bg-white/5 border-white/5 opacity-40'}`}>
                ...
                <button 
                    ...
                    ${isBridgeActive 
                        ? 'bg-[#A9DDD3] text-[#010101] shadow-[0_0_30px_rgba(169,221,211,0.2)] hover:bg-white' 
      ...
      {/* ACADEMY EXPLANATION (ПОКАЗЫВАЕМ ПОСЛЕ КЛИКА) */}
      <AnimatePresence>
        {isBridgeActive && (
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 30 }} className="fixed bottom-[420px] right-[8%] z-[60] p-10 bg-[#010101]/95 backdrop-blur-3xl border border-[#A9DDD3]/30 rounded-[3rem] max-w-2xl shadow-[0_0_60px_rgba(169,221,211,0.2)] pointer-events-none">
             <div className="flex items-center space-x-3 mb-6"><div className="p-3 bg-[#A9DDD3]/10 rounded-xl text-[#A9DDD3]"><Wifi size={24} className="animate-pulse" /></div><span className="text-sm font-mono text-[#A9DDD3] uppercase tracking-[0.3em] font-bold">Protocol Analysis // World Sensor</span></div>
             <div className="space-y-6 text-[#E8E3D5]"><p className="text-xl font-bold italic border-b border-white/10 pb-4">"Direct L1-to-Web2 connectivity is established."</p>
                <p className="text-base leading-relaxed opacity-90">By baking HTTPS-bridges natively into the consensus, we eliminate the 3rd-party "Oracle Tax".</p>
                <div className="grid grid-cols-2 gap-6 mt-8">
                    <div className="p-6 bg-white/5 rounded-3xl border border-white/10"><p className="text-[11px] font-mono text-[#A9DDD3] uppercase mb-2">Native Latency</p><p className="text-2xl font-black">12ms</p></div>
                    <div className="p-6 bg-white/5 rounded-3xl border border-white/10"><p className="text-[11px] font-mono text-[#A9DDD3] uppercase mb-2">Security Tier</p><p className="text-2xl font-black">Direct</p></div>
                </div>
             </div>
          </motion.div>
        )}
