...
import React, { useState, useEffect } from 'react'; // Исправлено: добавлены useState и useEffect
import { motion, AnimatePresence } from 'framer-motion';
import { ArrowLeft, Globe, Zap, ShieldCheck, Trophy, Cpu, CheckCircle, Activity, Sparkles, Database, Wifi, Award } from 'lucide-react';
import Link from 'next/link';
import { useAcademy } from "../../context/AcademyContext"; // Подключение связи с Агентом

export default function RialoEdgeMission() {
  const { setIsFocused } = useAcademy(); // Доступ к управлению Агентом
  ...
  useEffect(() => {
    ...
    // Сбрасываем фокус Агента при уходе со страницы
  ...
  const toggleBridge = () => {
    ...
    setIsFocused(newState); // Агент приближается к экрану при активации моста!
  };
  ...
  const handleComplete = () => {
    ...
    setIsCompleted(true);
    setIsFocused(false); // Агент возвращается на место после миссии
  };
      ...
      <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        <div className="lg:col-span-2 space-y-8">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-[#010101]/80 backdrop-blur-2xl border border-white/10 rounded-[3rem] p-10 md:p-12 relative overflow-hidden shadow-2xl">
                ...
                <p className="text-rialo-mint font-mono text-[9px] tracking-[0.5em] mb-4 uppercase opacity-60 italic italic">Connectivity Layer // World Interface</p>
                ...
                {/* --- МОЩНЫЙ ВИЗУАЛИЗАТОР ПОТОКА ДАННЫХ --- */}
                                        ...
                                        <>
                                            ...
                                            {[...Array(8)].map((_, i) => (
                                                <motion.div
                                                    ...
                                                    className="absolute top-[-4px] w-3 h-2 bg-rialo-mint rounded-full blur-[1px] shadow-[0_0_10px_#A9DDD3]"
                                                />
                                            ))}
                                        </>
                                    )}
      ...
      <AnimatePresence>
        {isBridgeActive && (
          <motion.div 
            ...
            className="fixed bottom-40 right-48 z-[60] p-6 bg-rialo-black/90 backdrop-blur-xl border-l-4 border-rialo-mint rounded-r-2xl max-w-[240px] shadow-2xl pointer-events-none"
          >
             <div className="flex items-center space-x-2 mb-3">
                <Wifi size={14} className="text-rialo-mint animate-pulse" />
                <span className="text-[9px] font-mono text-rialo-mint uppercase tracking-widest">Neural Feed Active</span>
             </div>
             <p className="text-xs text-rialo-beige leading-relaxed italic font-medium">
                "Detecting native HTTPS packets. Latency: <span className="text-rialo-mint font-bold text-glow-mint">12ms</span>. We've bypassed the Oracle Tax, Initiate."
             </p>
          </motion.div>
        )}
      </AnimatePresence>
