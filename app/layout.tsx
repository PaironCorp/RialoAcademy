...
function GlobalUI({ children }: { children: React.ReactNode }) {
  ...
  return (
    <body className="antialiased bg-[#010101] text-[#E8E3D5] overflow-x-hidden font-sans">
      ...
      {/* 1. ЖИВОЙ ФОН: Прозрачность ниже на мобилках для читаемости */}
      <div className="fixed inset-0 z-0 pointer-events-none opacity-20 md:opacity-40">
        <NexusBackground />
      </div>

      {/* 2. АДАПТИВНЫЙ ХЕДЕР: Больше отступов на мобилке */}
      ...
      {/* 3. КОНТЕНТ: pt-32 исправляет наложение лого на Economics */}
      <main className="relative z-10 pt-32 md:pt-0">
        {children}
      </main>

      {/* 4. ВИДЖЕТ ЧАТА: Умная адаптация */}
      <div className="fixed bottom-6 right-6 md:bottom-12 md:right-12 z-50 flex flex-col items-end transform-gpu">
          ...
          {/* ТРИГГЕР: Кнопка-Викинг */}
          <motion.div 
            onClick={() => setIsChatOpen(!isChatOpen)}
            className="relative cursor-pointer pointer-events-auto"
          >
              {!isChatOpen && (
                <div className="absolute -top-1 -left-1 w-5 h-5 bg-[#A9DDD3] rounded-full flex items-center justify-center text-black z-20 animate-bounce">
                  <MessageSquare size={10} fill="black" />
                </div>
              )}
              <Image 
                src="/avatar.png" 
                alt="Mentor" 
                width={isChatOpen ? 100 : 70} 
                height={isChatOpen ? 100 : 70} 
                className={`object-contain transition-all md:w-40 md:h-40 ${!isChatOpen && 'brightness-50'}`} 
              />
          </motion.div>
      </div>
