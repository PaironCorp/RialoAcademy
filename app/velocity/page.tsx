...
export default function VelocityMission() {
  ...
  const startRace = () => {
    ...
    // RIALO: 50ms - Мгновенный взрыв
    setTimeout(() => {
        setRaceProgress(prev => ({ ...prev, rialo: 100 }));
        // После финиша Rialo переходим в режим затухания
    ...
    // ETHEREUM: Имитация вечности
    const ethInterval = setInterval(() => {
        setRaceProgress(prev => {
            if (prev.eth >= 12 || status === "COOLED") { clearInterval(ethInterval); return prev; }
  ...
  return (
    <main className="min-h-screen p-6 md:p-12 lg:pt-32 flex flex-col items-center relative z-10 overflow-hidden">
      ...
      {/* ЭФФЕКТ ВСПЫШКИ ПРИ ФИНИШЕ RIALO */}
      <AnimatePresence>
        {raceProgress.rialo === 100 && status === "RACING" && (
            <motion.div 
                initial={{ opacity: 0 }} animate={{ opacity: 0.3 }} exit={{ opacity: 0 }}
                className="fixed inset-0 bg-[#A9DDD3] z-0 blur-[100px] pointer-events-none"
            />
        )}
      </AnimatePresence>
      ...
      <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-3 gap-8 items-start relative z-10">
        <div className="lg:col-span-2 space-y-6">
            <motion.div layout className={`bg-[#010101]/80 backdrop-blur-3xl border ${status === 'RACING' ? 'border-[#A9DDD3]/50' : 'border-white/10'} rounded-[3rem] p-10 md:p-12 shadow-2xl relative overflow-hidden transition-all duration-700`}>
                
                {/* ДИНАМИЧЕСКИЙ ГЛОУ-ФОН */}
                ...
                <p className="text-[#E8E3D5]/70 text-lg md:text-xl font-medium leading-relaxed max-w-xl mb-12 italic">
                    While others confirm blocks, Rialo has already finalized the world. <strong className="text-white">50ms</strong> isn't speed—it's a new dimension.
                </p>

                {/* --- THE ENERGY CORE RACE --- */}
                <div className="bg-white/[0.01] border border-white/5 rounded-[3.5rem] p-12 space-y-12 relative">
                    
                    {/* RIALO: THE SONIC BOOM */}
                    <div className="relative">
                        <div className="flex justify-between items-end mb-4">
                            <div className="flex items-center space-x-2">
                                <Zap size={16} className={raceProgress.rialo === 100 ? "text-[#A9DDD3] animate-pulse" : "text-white/20"} />
                                <span className="font-mono text-[11px] text-[#A9DDD3] uppercase tracking-[0.4em] font-black italic">RIALO OVERCLOCK [50ms]</span>
                            </div>
                            <span className={`text-[10px] font-mono ${raceProgress.rialo === 100 ? 'text-[#A9DDD3]' : 'text-white/20'}`}>
                                {raceProgress.rialo === 100 ? "BLOCK CONFIRMED" : "WAITING..."}
                            </span>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 opacity-40 group hover:opacity-60 transition-opacity">
                        <div className="space-y-3">
                            <span className="font-mono text-[9px] text-white/40 uppercase tracking-widest italic">Solana [400ms]</span>
                            <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                                <motion.div animate={{ width: `${raceProgress.sol}%` }} className="h-full bg-white/20" />
                            </div>
                        </div>
                        <div className="space-y-3">
                            <span className="font-mono text-[9px] text-white/20 uppercase tracking-widest italic">Ethereum [12s]</span>
                            <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                    ...
                    >
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-[#E8E3D5]">
                          <div className="space-y-4">
                             <p className="text-xl font-bold italic">"800% Faster than high-performance L1s."</p>
                             <p className="text-sm opacity-60 leading-relaxed font-medium">Rialo’s 50ms finality enables on-chain AI and high-frequency markets that were previously impossible on decentralized rails.</p>
                          </div>
                          <div className="p-8 bg-[#A9DDD3]/5 border border-[#A9DDD3]/20 rounded-[2.5rem] flex flex-col justify-center text-center">
                                ...
                                <p className="text-[9px] opacity-30 mt-4 italic font-mono">Consensus: Sovereign SVM High-Speed Core</p>
                          </div>
                       </div>
                    </motion.div>
                  )}
        ...
        <div className="space-y-6">
            <div className="bg-white/[0.02] border border-white/10 rounded-[2.5rem] p-10 backdrop-blur-md">
                ...
                <button 
                    ...
                    ${status === 'COOLED' ? 'bg-[#A9DDD3] text-[#010101] hover:bg-white shadow-xl shadow-[#A9DDD3]/20' : 'bg-white/5 text-white/20 cursor-not-allowed'}`}
                >
